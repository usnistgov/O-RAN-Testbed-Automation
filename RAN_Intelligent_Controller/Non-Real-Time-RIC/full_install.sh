...
# First, get the latest nonrtric submodule in the j-release branch
NONRTRIC_BRANCH=$(git -C nonrtric branch --show-current)
if [ "$NONRTRIC_BRANCH" != "j-release" ]; then
    echo
...
cd $BASE_DIR/dep/
RIC_YAML_FILE_PATH="RECIPE_EXAMPLE/NONRTRIC/example_recipe.yaml"
RIC_YAML_FILE_PATH_MODIFIED="RECIPE_EXAMPLE/NONRTRIC/example_recipe_MODIFIED.yaml"
...
sudo $BASE_DIR/install_scripts/./revise_example_recipe_yaml.sh "$RIC_YAML_FILE_PATH_MODIFIED"
...
echo "The Non Real-Time RAN Intelligent Controller installation completed successfully. Starting k9s in 10 seconds..."
...
# cd $BASE_DIR/dep/nonrtric/test/auto-test
# https://docs.o-ran-sc.org/projects/o-ran-sc-portal-nonrtric-controlpanel/en/latest/developer-guide.html

if ! command -v docker-compose &>/dev/null; then
    echo
    echo "Installing docker-compose"
    curl -SL https://github.com/docker/compose/releases/download/v2.30.1/docker-compose-linux-x86_64 | sudo tee /usr/local/bin/docker-compose >/dev/null
...
echo "The Non RT-RIC Control Panel is now running. Check the logs directory for its status: logs/controlpanel_stdout.txt"