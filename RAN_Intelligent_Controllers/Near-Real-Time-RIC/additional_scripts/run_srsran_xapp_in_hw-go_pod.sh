...
# This script will ask the user to select an xApp to run, then install it from
# within the hw-go pod in the RIC and directing the output to the console.
################################################################################
...
if command -v kubectl &>/dev/null; then
    POD_NAME=$(kubectl get pods -n ricxapp --no-headers | grep "ricxapp-hw-go" | grep "Running" | awk '{print $1}' | head -n 1)
    if [[ -z "$POD_NAME" ]]; then
        echo "No running hw-go pod was found."
        exit 1
    fi
    ...
    echo "Writing \"$SCRIPT_NAME\" to $POD_NAME..."
    kubectl cp "$SCRIPT_PATH" ricxapp/$POD_NAME:/run_srsran_xapp.sh

    echo "Running script in $POD_NAME..."
    kubectl exec -it "$POD_NAME" -n ricxapp -- /bin/sh -c "./run_srsran_xapp.sh --submgr_ip=\"$SUBMGR_IP\" --rmr_port=\"$RMR_PORT\""

    exit 0
fi

################################################################################
# The following code will be executed inside the xApp
################################################################################

echo "Script should now be running within the xApp."
echo
echo "List of files:"
ls
...
if [ "$SCRIPT_DIR" = "/" ]; then
    ...
    echo "Current script directory is not in root. Exiting."
    exit 1
fi
...
if [ ! -f /usr/local/lib/librmr_si.so ]; then
    ...
    if [ ! -f /usr/local/lib/librmr_si.so ]; then
        if [ -n "$LIB_PATH" ] && [ -f "$LIB_PATH" ]; then
            cp "$LIB_PATH" /usr/local/lib/librmr_si.so
        else
...
if [ ! -f /usr/local/lib/libriclibe2ap.so ]; then
    ...
    if [ ! -f /usr/local/lib/libriclibe2ap.so ]; then
        if [ -n "$LIB_PATH" ] && [ -f "$LIB_PATH" ]; then
            cp "$LIB_PATH" /usr/local/lib/libriclibe2ap.so
        else
...
if [ "$SELECTED_XAPP_PATH" = "kpm_mon_xapp.py" ]; then
    ...
    SELECTED_APP_ARGS="--ue_ids 0 --metrics=DRB.UEThpDl,DRB.UEThpUl --kpm_report_style=5 --http_server_port=$HTTP_SERVER_PORT --rmr_port=$RMR_PORT --e2_node_id=$E2_NODE_ID"
