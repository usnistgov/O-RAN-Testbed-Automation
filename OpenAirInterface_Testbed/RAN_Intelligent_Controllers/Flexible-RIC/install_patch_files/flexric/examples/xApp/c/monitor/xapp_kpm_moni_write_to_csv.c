...
static
void csv_append_name_to_csv_header(byte_array_t name)
{
...
static
void csv_append_int_to_csv_line(meas_record_lst_t meas_record)
{
  size_t current_len = strlen(csv_line_buffer);

  // Don't overflow the buffer
  if (current_len < sizeof(csv_line_buffer) - 32) { // reserve space for int/float
...
static
void csv_append_real_to_csv_line(meas_record_lst_t meas_record)
{
  size_t current_len = strlen(csv_line_buffer);
...
{
  if (!csv_wrote_header) csv_append_name_to_csv_header(name);
