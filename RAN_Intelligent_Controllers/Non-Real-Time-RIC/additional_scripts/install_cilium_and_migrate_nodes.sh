...
metadata:
  name: isolate-nonrtric-communication
  namespace: nonrtric
spec:
  endpointSelector:
    matchLabels:
      "k8s:io.kubernetes.pod.namespace": nonrtric
  ingress:
    - fromEndpoints:
      - matchLabels:
      ...
      - matchLabels:
          "k8s:io.kubernetes.pod.namespace": nonrtric
      - matchLabels:
          "k8s:io.kubernetes.pod.namespace": onap
  egress:
    - toEndpoints:
      - matchLabels:
          "k8s:io.kubernetes.pod.namespace": kube-system
...
kind: CiliumNetworkPolicy
metadata:
  name: isolate-onap-communication
  namespace: onap
spec:
  endpointSelector:
    matchLabels:
...
# Prevent Non-RT RIC pods from making modifications such as changing their labels to bypass enforcement
# If the nonrtric namespace exists, create a Role for Non-RT RIC read restriction, and a RoleBinding for the default ServiceAccount
NONRTRIC_READ_RESTRICTION_FILE="$HOME/.kube/nonrtric-read-restriction.yaml"
NONRTRIC_ROLE_BINDING_FILE="$HOME/.kube/nonrtric-role-binding.yaml"
NAMESPACE="nonrtric"
...
kind: Role
metadata:
  name: nonrtric-read-restriction
  namespace: $NAMESPACE
rules:
- apiGroups: [""]
  resources: ["pods"]
...
EOF
        ...
        if ! kubectl apply -f $NONRTRIC_READ_RESTRICTION_FILE; then
            ...
            echo "Non-RT RIC read restriction Role successfully applied."
        fi
    else
        echo "Non-RT RIC read restriction Role file already exists. No changes made."
    fi
...
kind: RoleBinding
metadata:
  name: nonrtric-read-restriction-binding
  namespace: $NAMESPACE
subjects:
- kind: ServiceAccount
...
EOF
        ...
        if ! kubectl apply -f $NONRTRIC_ROLE_BINDING_FILE; then
            ...
            echo "Non-RT RIC RoleBinding successfully applied."
        fi
    else
        echo "Non-RT RIC RoleBinding file already exists. No changes made."
    fi
else
